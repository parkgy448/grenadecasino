...
(function () {
    ...
    const dlg = popup.querySelector(".tetherPopup__dialog");
    const imgEl = popup.querySelector(".tetherPopup__img");
    const btnX = popup.querySelector(".tetherPopup__close");
    const btnOk = popup.querySelector("#tetherOk");
    ...
    // 다른 스크립트에서 필요하면 호출 가능: openTetherPopup(url?)
    window.openTetherPopup = function (url) {
        if (shouldHide()) return;
        imgEl.src = url && /^https?:\/\/.+\.(png|jpe?g|gif|webp|svg)$/i.test(url) ? url : DEFAULT_IMG;
        popup.style.display = "flex";
        lockScroll(true);
    };

    function close() {
        if (todayCb.checked) rememberToday();
    ...
    window.addEventListener("load", () => {
        if (!shouldHide()) window.openTetherPopup();
    });
})();
